...
class Piece:
    ...
    def __str__(self):
        # מחזיר ייצוג מחרוזתי כמו "wQ" או "bK"
        return f"{self.color}{self.piece_type}"

    def is_valid_step(self, board_grid, from_cell, to_cell):
     
        raise NotImplementedError

    def _check_blocking(self, board_grid, from_cell, to_cell):
        ...
        while (current_row, current_col) != (r_new, c_new):
            ...
            if board_grid[current_row][current_col] != cg.EMPTY_CELL:
                return True
...
class Rook(Piece):
    ...
    def is_valid_step(self, board_grid, from_cell, to_cell):
        ...
        if delta_row == 0 or delta_col == 0:
            return not self._check_blocking(board_grid, from_cell, to_cell)
        return False
...
class Pawn(Piece):
    ...
    def is_valid_step(self, board_grid, from_cell, to_cell):
        ...
        start_row = len(board_grid) - 1 if self.color == cg.COLOR_WHITE else 0
        delta_row = r2 - r1
        delta_col = abs(c2 - c1)
        if delta_col == 0 and  (delta_row==direction or (delta_row==2*direction and r1 == start_row)):
            if board_grid[r2][c2] != cg.EMPTY_CELL:
                return False
            if delta_row == 2 * direction:
                if self._check_blocking(board_grid, from_cell, to_cell):
                    return False
            return True
        
        if delta_col == 1 and delta_row == direction:
            target_piece = board_grid[r2][c2]
