...
class Img:
    ...
    @staticmethod
    def blank(width: int, height: int, color: tuple[int, int, int] = (0, 0, 0)) -> "Img":
        """יוצרת קנבס ריק (3 ערוצים, BGR) בגודל נתון, מלא בצבע אחיד."""
        canvas = Img()
        canvas.img = np.zeros((height, width, 3), dtype=np.uint8)
        canvas.img[:] = color
        return canvas
    ...
    def dim(self, alpha, color=(0, 0, 0)):
        """מכהה את כל התמונה ע"י מיזוג עם צבע אחיד. alpha=0 ללא שינוי, alpha=1 מלא בצבע."""
        overlay = np.full_like(self.img, color, dtype=self.img.dtype)
